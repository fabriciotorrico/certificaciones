...
<?php/*
   $database = pg_connect("host=10.10.10.11 port=5432 dbname=qmap user=postgres password=fabricio");
  ...
  // pg_lo_export($database, $oid, '/tmp/lob.dat');
   pg_lo_export($database, $oid, 'C:/abc/a.dat');
...
user=postgres password=rularv"); //connectionstring
  ...
  $dbconn = pg_pconnect("host=10.10.10.11 port=5432 dbname=$dbname
user=postgres password=fabricio"); //connectionstring
  if (!$dbconn) {
    echo "Can't connect.\n";
  exit;
  }
